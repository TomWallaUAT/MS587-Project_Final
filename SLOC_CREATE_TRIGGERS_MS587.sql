...
BEGIN
	...
	BEGIN
		...
		(
			Select 
				dbo.fnc_GetActivityID('DEVICE_PORT_DEL') as ACT_TYP_ID,
				DPC.DEV_ID,
				RTRIM('<<DEVICE PORT DELETED>>  -  [DEV_ID] - ( ''' + CAST(DPC.[DEV_ID] as varchar(60)) + ''' ), ' + 
				CASE when UPDATE(PRT_ID) THEN '[PRT_CFG_ID] - ( ' + CAST(ISNULL(D.PRT_CFG_ID,'--') as varchar(10)) + ' ), ' ELSE '' END +  
				CASE when UPDATE(PRT_ID) THEN '[PRT_ID] - ( ' + CAST(ISNULL(D.PRT_ID,'--') as varchar(3)) + ' ), ' ELSE '' END
				) as ACT_DESC,
				D.LST_UPDT_USER_ID
			FROM
...
--CREATE TRIGGER (OR ALTER was added in SQL Server 2016, Wont work below 2016) 
CREATE OR ALTER TRIGGER TR_DEVICE_PORT_CONFIG_DELETE
	...
	BEGIN
		...
		(
			Select 
				dbo.fnc_GetActivityID('DEVICE_PORT_DEL') as ACT_TYP_ID,
				D.DEV_ID,
				RTRIM('<<DEVICE PORT DELETED>>  -  [DEV_ID] - ( ''' + CAST(D.[DEV_ID] as varchar(60)) + ''' ), ' +
				'[PRT_CFG_ID] - (' + CAST(D.PRT_CFG_ID as varchar(10)) + '),') as ACT_DESC,
				D.LST_UPDT_USER_ID
			FROM
				DELETED D		
		) DRV
...
BEGIN
	...
	--DONT CARE ABOUT THE 4 FIELDS FOR THE TIMESTAMP RECORDS - THESE SHOULDN'T TRIGGER AN AUDIT RECORD (IGNORED)
	--IF NOT UPDATE(LST_UPDT_TS) AND NOT UPDATE(LST_UPDT_USER_ID) 
	IF UPDATE(PRT_CNT) OR UPDATE(PRT_GNDR) 
	BEGIN
		--BUILDS DETAILED LIST OF WHAT CHANGED FOR THE AUDIT RECORD
		INSERT INTO dbo.AUDIT_ACTIVITY (ACT_TYP_ID, ACT_DEV_ID, ACT_DESC, REC_CRTE_USER_ID)
		Select
		...
		(
			Select 
				dbo.fnc_GetActivityID('DEVICE_PORT_UPD') as ACT_TYP_ID,
				I.DEV_ID,
				...
				CASE when I.PRT_GNDR <> D.PRT_GNDR THEN '[PRT_GNDR] - ( ''' + D.PRT_GNDR + ''' -> ''' + I.PRT_GNDR  + ''' ), ' ELSE '' END 
				) as ACT_DESC,
				I.LST_UPDT_USER_ID
			FROM
				INSERTED I INNER JOIN DELETED D
